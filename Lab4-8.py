...
# Implement insertion-sort on a positional list
...
#----- Lab4.8 Implement insertion-sort on a positional list ---------------
...
def insertion_sort(L):
    ...
    if len(L) > 1:                        # otherwise, no need to sort it
        ...
        while marker != L.last():
            ...
            value = pivot.element()
            if value < marker.element():  # must relocate (sort) pivot
                walk = marker             # find leftmost value greater than value
                while walk != L.first() and value < L.before(walk).element():
                    walk = L.before(walk)
                # delete and relocate pivot
                L.delete(pivot)            
                L.add_before(walk, value)  # reinsert value before walk
            else:                          # pivot is alreadly sorted
